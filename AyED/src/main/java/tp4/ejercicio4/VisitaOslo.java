    ...
    public static List<String> paseoEnBici(Graph<String> lugares, String destino, int maxTiempo, List<String> lugaresRestringidos){
        ...
        if(lugares!=null && !lugares.isEmpty()) {
            for (String s : lugaresRestringidos) {
                Vertex<String> aux = lugares.search(s);
                if (aux != null) {
            ...
            if(dest!=null && origen!= null){
                paseoEnBici(lugares,origen,dest,maxTiempo,l,marca);
            }
        }
        return l;
    }

    private static boolean paseoEnBici(Graph<String> lugares,Vertex<String>origen,Vertex<String>destino,int maxTiempo,List<String>l,boolean[] marca){
        marca[origen.getPosition()]=true;
        l.add(origen.getData());
        if(origen == destino && maxTiempo>=0){
            return true;
        }else{
            List<Edge<String>> aristas = lugares.getEdges(origen);
            for(Edge<String> e : aristas){
                if(!marca[e.getTarget().getPosition()] && maxTiempo - e.getWeight() >=0){
                    boolean exito = paseoEnBici(lugares,e.getTarget(),destino,maxTiempo-e.getWeight(),l,marca);
